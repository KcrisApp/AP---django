<template>
  <main>
    ...
    <div class="p-4 w-full">
      ...
      <div v-if="isLoading">
        <BoardForm v-show="showForm" @close-modal="togleForm" @save-data="updateBoard" v-bind:boardArr="board" />
        <OrderForm v-show="showModal" v-bind:board_id="board.id" @close-modal="togleModal" @save-data="updateOrder" />
        <ModalImg v-show="showFormImg" @close-alert="togleImgForm" v-bind:board_id="board.id" v-bind:side="sideType"
          @save-img="imgUpdate" />
      </div>
      ...
      <div v-if="board.alert_info" class="  text-lg  rounded-xl mt-4 bg-red-50 text-red-700 shadow-md">

        <div class="flex flex-wrap items-center px-4 pt-4 gap-2 ">
          <font-awesome-icon icon="bell" class=" text-red-700" />
          <span
            class="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-sm font-medium text-red-700 ring-1 ring-inset ring-red-600/10">Alert</span>
      ...
      <div class=" bg-slate-50 p-4 rounded-xl">
        ...
        <div class="flex gap-4  flex-wrap justify-start">
          <div class="col">
            ...
            <button v-if="store.permissionAccess"
      ...
      <div class="flex justify-center mt-5">
        <button class="hover:border-blue-500 text-blue-950 font-semibold hover:text-white py-1 px-4 rounded" :class="showSteps
            ? 'hover:bg-red-500'
      ...
      <div v-if="showSteps">
        ...
        <div>
          <draggable class="flex flex-wrap justify-center mt-10" v-model="steps" group="step" item-key="id" tag="ul"
            drag-class="drag" ghost-class="ghost" @change="onChangeStepOrder" :disabled="!store.permissionAccess">
            <template #item="{ element, index }">
              <StepCard :id="element.uuid" :step="element" @delete-step="deleteStep(element.uuid)"
                @mod-step="modStep(element)" :index="index" />
            </template>
          </draggable>
        </div>
        <StepForm v-show="showStepsForm" :board="orderNumber" @close-modal="togleStepForm" @save-data="saveDataStep" />
        <StepForm v-if="showModStepsForm" :steps="singleStep" :board="orderNumber"
          @close-modal="showModStepsForm = false" @save-data="updateSingleStep" />
      </div>
    </div>
  </main>
</template>
