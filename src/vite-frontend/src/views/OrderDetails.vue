<template>
  <main>
    ...
    <div v-if="onLoad" class="p-4 w-full" v-for="o in order">

      <Alert v-show="showAlert" icon-type="error" title="Attenzione" message="Sei sicuro di voler eliminare la scheda?"
        @close-alert="togleAlert" @confirm="deleteOrder(o.order_number)" />
      <OrderForm v-show="showModal" v-bind:order_id="o.board" v-bind:order="o" @close-modal="togleModal"
        @save-data="updateOrder" />
      <ModalFile v-show="showFormFile" @close-alert="togleModalFile" v-bind:order_id="o.id" :file_type="fileType"
      ...
      <div class="bg-gray-100 my-4 p-4 rounded-md text-sm">
        ...
        <div class="flex gap-6">
          <router-link v-if="store.isOfficeUser" :to="{
      ...
      <div class="flex gap-2">
        ...
        <div class="bg-gray-100 my-2 p-4 rounded-md text-sm flex-1">
          <div class="flex justify-between">
            <p><b>Topographic:</b></p>
            <font-awesome-icon v-if="store.permissionAccess" icon="circle-plus" class="text-2xl hover:text-green-700"
              @click="togleModalFileTopographic" />
          </div>

          <hr class="my-2" />
          <p v-if="o.order_file_topographic">
        ...
        <div class="bg-gray-100 my-2 p-4 rounded-md text-sm flex-1">
          <div class="flex justify-between">
            <p><b>ODB++:</b></p>
            <font-awesome-icon v-if="store.permissionAccess" icon="circle-plus" class="text-2xl hover:text-green-700"
              @click="togleModalFileOdb" />
          </div>

          <hr class="my-2" />
          <p v-if="o.order_file_odb">
        ...
        <div class="bg-gray-100 my-2 p-4 rounded-md text-sm flex-1">
          <div class="flex justify-between">
            <p><b>Gerber:</b></p>
            <font-awesome-icon v-if="store.permissionAccess" icon="circle-plus" class="text-2xl hover:text-green-700"
              @click="togleModalFileGerber" />
          </div>

          <hr class="my-2" />
          <p v-if="o.order_file_gerber">
        ...
        <div class="bg-gray-100 my-2 p-4 rounded-md text-sm flex-1">
          <div class="flex justify-between">
            <p><b>Schematics:</b></p>
            <font-awesome-icon v-if="store.permissionAccess" icon="circle-plus" class="text-2xl hover:text-green-700"
              @click="togleModalFileSchematics" />
          </div>

          <hr class="my-2" />
          <p v-if="o.order_file_schematics">
      ...
      <div class="bg-gray-100 my-2 p-4 rounded-md text-sm">
        <div class="flex justify-between">
          <p><b>Distinta:</b></p>
          <router-link v-if="store.permissionAccess"
        ...
        <p>
          ...
          <router-link v-if="store.permissionAccess" :to="{ name: 'bom', params: { order_number: o.order_number } }">
      ...
      <div class="flex gap-2 mt-4 flex-wrap">
        <router-link :to="{ name: 'smt-details', params: { smt_number: o.order_smt } }"
          class="py-2 px-6 border border-blue-950 rounded-md bg-gray-50 hover:bg-blue-900 hover:text-white">
          SMT
        </router-link>
        <router-link :to="{ name: 'welding-details', params: { welding_number: o.order_welding } }"
          class="py-2 px-6 border border-blue-950 rounded-md bg-gray-50 hover:bg-blue-900 hover:text-white">
          Saldatura
        </router-link>
        <router-link :to="{
        ...
        <router-link :to="{ name: 'shipping-details', params: { order_id: o.id, order_qta: o.order_quantity } }"
          class="py-2 px-6 border border-blue-950 rounded-md bg-gray-50 hover:bg-blue-900 hover:text-white">
          Spedizioni
        </router-link>
