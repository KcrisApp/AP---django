<template>
  ...
  <div class="mb-3 border rounded-md  p-5">
  ...
  <button v-if="!tableShow" @click="save" class="btn bg-emerald-500 px-4 py-2 rounded-md m-2 text-white text-sm">Salva
    piazzamenti
  </button>
  <button v-if="!tableShow" @click="exportFile"
    class="btn bg-cyan-600 px-4 py-2 rounded-md m-2 text-white text-sm">Download placement
  </button>
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    ...
    <table v-if="tableShow"
      ...
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr>
          <th v-for="n in colNumber" scope="col" class="px-6 py-3 text-md">
            <select @change="selCol($event)" :id="n - 1" :ref="n - 1">
              <option value="0">-</option>
              <option value="1">Tipo magazzino</option>
              <option value="2">Componente/Qta</option>
              <option value="3">Desc.Componente</option>
              <option value="4">Posizione stock</option>
            </select>
          </th>
        </tr>
      </thead>
    ...
    <table v-if="!tableShow"
      ...
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr>
          ...
          <th scope="col" class="px-6 py-3">Posizione stock</th>
        </tr>
      </thead>
      <tbody>
...
const colOrder = ref([])
const typeCol = ['', 'border-red-500 border-solid border-4', 'border-orange-500 border-solid border-4', 'border-grey-500 border-solid border-4', 'border-blue-500 border-solid border-4']
const bomResult = ref([])
...
const createBom = () => {
  rowSelectArray.value.forEach(element => {
    ...
    let arrToPush = [element[colOrder.value.indexOf('1')], element[colOrder.value.indexOf('2')], element[colOrder.value.indexOf('3')], element[colOrder.value.indexOf('4')]]
...
const save = () => {
  ...
  rowSelectArray.value.forEach(element => {
    ...
    // console.log(`Description: ${element[colOrder.value.indexOf('4')]}`)
    // console.log(`Value: ${element[colOrder.value.indexOf('5')]}`)
    let jsonToPush = { 'type': element[colOrder.value.indexOf('1')], 'component': element[colOrder.value.indexOf('2')], 'desc': element[colOrder.value.indexOf('3')], 'position': element[colOrder.value.indexOf('4')], 'note': "", 'check': false }
...
async function sentData(arr_to_save) {
  ...
  try {
    const response = await axios({
      ...
      url: endpoint,
      data: {
        order_placement_pick_and_place: JSON.stringify(arr_to_save),
      },
