  ...
  <main>
    <div class="p-4 w-full">
      ...
      <div class="flex justify-between my-4 ">
        ...
        <h1 class="text-md">
          <span v-if="smt.status" class="text-white bg-green-700 p-1 px-2 rounded-md">COMPLETATO</span>
      ...
      <div class="flex justify-between p-4 bg-gray-100 rounded-lg mt-2">
        ...
        <h1 class="text-sm">
          <span v-if="smt.check_status" class="text-white bg-green-700 p-1 px-2 rounded-md">PASS</span>
          <span v-else class="text-white bg-red-700 p-1 px-2 rounded-md">FAIL</span>
        </h1>
      </div>
    ...
    <SmtForm v-show="showForm" @close-modal="togleSmtForm" @save-data="updateSmt" v-if="isLoading" :smt="smt" />
  </main>
</template>
<script setup>
