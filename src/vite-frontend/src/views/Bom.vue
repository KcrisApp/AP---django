...
// Create xlsx order_bom
  ...
  const addToList = (id) =>{
    console.log(id)
    console.log(order_bom.value[id].check)
    
    order_bom.value[id].check = !order_bom.value[id].check
    updateData()
}
...
<div class="flex">
  <button
  @click="exportFile"
 class="btn bg-emerald-500 px-4 py-2 rounded-md m-2 text-white text-sm">Download
</button>
  <button
 @click="resetAll"
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
  ...
  <!-- <NoteForm v-model="note"  v-if="showEditNote" @close-modal="closeModal"/> -->
    ...
    class="w-full divide-y divide-gray-200 dark:divide-neutral-700 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 overflow-x-auto over mt-4">
        ...
        <tbody>
          <tr 
          v-for="(row, idx) in order_bom" 
          :key="idx" 
          @dblclick ="addToList(idx)"
          class="hover:text-white hover:bg-blue-950"
