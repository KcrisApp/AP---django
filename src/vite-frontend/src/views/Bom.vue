<template>
      ...
      <div class="m-2">
        <div class="relative">
          ...
          <input type="search" id="default-search"
            ...
            placeholder="Cerca drawing ref..." v-model="search" @keyup="filterSearch" required />
        </div>
      </div>
      <button @click="trigModal" class="btn bg-slate-300 hover:bg-slate-400 px-4 py-2 rounded-md m-2 text-sm">Filter
      </button>
  ...
  <div v-show="showModalOption" class="bg-slate-100 my-2 p-4 rounded-md text-sm border shadow-md">
    <div class="flex justify-between">
      <p><b>Filter:</b></p>
      <p @click="resetFilter" class="underline cursor-pointer">reset filter</p>
    </div>

    <hr class="my-2" />
    <div class="flex justify-start gap-2">
      <button @click="filterSmdActive" :class="filterSmt ? 'bg-blue-700 text-white' : ''"
      ...
      <button @click="filterThtActive" :class="filterTht ? 'bg-green-600 text-white' : ''"
      ...
      <button @click="filterSortByDrowing"
  ...
  <div class="flex gap-2">
    <span
      class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">Codici
      totali: {{ codeCount }}</span>
    <span
      class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">Righe
      selezionate: {{ codeCheckedItem }}</span>
  </div>
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <table
      ...
      <tbody>
        <tr v-for="(row, idx) in colOrder_bom" :key="idx" @dblclick="addToList(row.id)"
          class="hover:text-white hover:bg-gray-400" :class="row.check ? ' text-white bg-blue-950 font-bold' : ''">
          <td class=" border-b dark:border-gray-700">
          <th scope="row" class="px-6 py-4 font-medium  whitespace-nowrap dark:text-white0">{{ row.drw }}
          </th>
          </td>
          <td class=" border-b dark:border-gray-700">
          <th scope="row" class="px-6 py-4 font-medium  whitespace-nowrap dark:text-white0">{{ row.pn }}
          </th>
          </td>
          <td class=" border-b dark:border-gray-700">
          <th scope="row" class="px-6 py-4 font-medium  whitespace-nowrap dark:text-white0">{{ row.qta }}
          </th>
          </td>
          <td class=" border-b dark:border-gray-700">
